...
class DDPM(BaseModel):
    ...
    def print_network(self):
        ...
        if isinstance(self.cond_ddpm, nn.DataParallel):
            net_struc_str = '{} - {}'.format(self.cond_ddpm.__class__.__name__, self.cond_ddpm.module.__class__.__name__)
        else:
            net_struc_str = '{}'.format(self.cond_ddpm.__class__.__name__)
        logger.info('Network conditional DDPM structure: {}, with parameters: {:,d}'.format(net_struc_str, n))
        logger.info(s)

    def save_network(self, epoch, iter_step):
        diffusion_path = os.path.join(self.opt['path']['checkpoint'], 'I{}_E{}_diff.pth'.format(iter_step, epoch))
