...
class GaussianDiffusion(nn.Module):
    def __init__(self, diffusion_net, image_size, channels=3, loss_type='l1', conditional=True, schedule_opt=None):
        super().__init__()
    ...
    def set_noise_schedule(self, schedule_opt, device):
        ...
        self.sqrt_gamma_tmin1 = np.sqrt(np.append(1., gamma))
        ...
        posterior_var = betas * (1. - gamma_tmin1) / (1. - gamma) # TODO
    ...
    def predict_start_from_noise(self, x_t, t, noise):
        return self.sqrt_frac_gamma[t] * x_t - self.sqrt_frac_gamma_min1[t] * noise # TODO
    ...
    def p_mean_variance(self, x, t, clip_denoised: bool, noisy_img=None): 
        ...
        noise_level = torch.FloatTensor([self.sqrt_gamma_tmin1[t+1]]).repeat(batch_size, 1).to(x.device)
        if noisy_img is not None:
            x_recon = self.predict_start_from_noise(x, t=t, noise=self.diffusion_net(torch.cat([noisy_img, x], dim=1), noise_level)) # TODO
        else:
            x_recon = self.predict_start_from_noise(x, t=t, noise=self.diffusion_net(x, noise_level))
        if clip_denoised:
            x_recon.clamp_(-1., 1.)
        model_mean, log_posterior_var = self.q_posterior(x_first=x_recon, x_t=x, t=t)
        return model_mean, log_posterior_var

    @torch.no_grad()
    def p_sample(self, x, t, clip_denoised=True, noisy_img=None):
        model_mean, log_model_var = self.p_mean_variance(x=x, t=t, clip_denoised=clip_denoised, noisy_img=noisy_img)
    ...
    def q_sample(self, x_first, continuous_sqrt_gamma, noise=None):
        ...
        return (continuous_sqrt_gamma * x_first + (1 - continuous_sqrt_gamma**2).sqrt() * noise)

    def p_losses(self, x_in, noisy_img, noise=None):
        x_first = x_in
        [b, c, h, w] = x_first.shape
        t = np.random.randint(1, self.timesteps_n + 1)
        continuous_sqrt_gamma = torch.FloatTensor(np.random.uniform(self.sqrt_gamma_tmin1[t-1], self.sqrt_gamma_tmin1[t], size=b)).to(x_first.device)
        continuous_sqrt_gamma = continuous_sqrt_gamma.view(b, -1)
        noise = default(noise, lambda: torch.randn_like(x_first))
        x_noisy = self.q_sample(x_first=x_first, continuous_sqrt_gamma=continuous_sqrt_gamma.view(-1, 1, 1, 1), noise=noise)
        if not self.cond:
            x_recon = self.diffusion_net(x_noisy, continuous_sqrt_gamma)
        else: 
            x_recon = self.diffusion_net(torch.cat([noisy_img, x_noisy], dim=1), continuous_sqrt_gamma) # TODO
