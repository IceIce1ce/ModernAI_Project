...
if __name__ == '__main__':
    ...
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=opt['datasets']['train']['batch_size'], shuffle=True)
    train_dataset_size = len(train_loader)
    print("train dataset size: ", train_dataset_size)
    val_dataset = Data_set(opt['datasets']['val'], phase='train')
    val_loader = torch.utils.data.DataLoader(val_dataset, batch_size=opt['datasets']['train']['batch_size'], shuffle=True)
    val_dataset_size = len(val_loader)
    print("test dataset size: ", val_dataset_size)
    test_dataset = Data_set(opt['datasets']['test'], phase='val')
    test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=opt['datasets']['test']['batch_size'], shuffle=True)
    ...
    diffusion.set_noise_schedule(opt['model']['beta_schedule'][opt['phase']], schedule_phase=opt['phase'])
    if opt['phase'] == 'train':
        while current_step < n_iter:
            ...
            for epoch_i, data in enumerate(train_loader):
                ...
                if current_step % opt['train']['val_freq'] == 0:
                    ...
                    if wandb_logger:
                        wandb_logger.log_metrics({'validation/val_psnr': avg_psnr, 'validation/val_ssim': avg_ssim, 'validation/val_step': val_step})
                        val_step += 1
            if wandb_logger:
        ...
        tb_logger.add_scalar('ssim', avg_ssim, current_step)
        if wandb_logger:
            wandb_logger.log_metrics({'test/val_psnr': avg_psnr, 'test/val_ssim': avg_ssim})